...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/60 p-4" (click)="closed.emit()">
      <div
        class="w-full max-w-lg rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-2xl max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <form class="space-y-4" (ngSubmit)="submit()">
          <div>
            <label for="title" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.title' | translate }}</label>
            <input
              id="title"
              name="title"
              required
          ...
          <div>
            <label for="location" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.location' | translate }}</label>
            <input
              id="location"
              name="location"
              [(ngModel)]="location"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label for="startTime" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.startTime' | translate }}</label>
              <input
                id="startTime"
                name="startTime"
                type="time"
            ...
            <div>
              <label for="endTime" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.endTime' | translate }}</label>
              <input
                id="endTime"
                name="endTime"
                type="time"
          ...
          <div class="flex items-center gap-2">
            ...
            <label for="recurring" class="text-sm text-slate-700 dark:text-slate-300">{{ 'calendar.form.repeatsWeekly' | translate }}</label>
          </div>

          @if (recurring) {
            <div>
              <p class="text-sm text-slate-700 dark:text-slate-300 mb-2">{{ 'calendar.form.onDays' | translate }}</p>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label for="startDate" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.from' | translate }}</label>
                <input
                  id="startDate"
                  name="startDate"
                  type="date"
              ...
              <div>
                <label for="endDate" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.until' | translate }}</label>
                <input
                  id="endDate"
                  name="endDate"
                  type="date"
            ...
            <div>
              <label for="date" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.date' | translate }}</label>
              <input
                id="date"
                name="date"
                type="date"
          ...
          <div>
            <p class="text-sm text-slate-700 dark:text-slate-300 mb-2">{{ 'calendar.form.color' | translate }}</p>
          ...
          <div>
            <label for="description" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'calendar.form.notes' | translate }}</label>
            <textarea
              id="description"
              name="description"
              rows="2"
          ...
          <div class="flex items-center justify-between pt-2">
            @if (event()) {
              <button
                type="button"
                (click)="remove()"
                class="rounded-lg border border-red-200 dark:border-red-900 text-red-600 dark:text-red-400 px-4 py-2 text-sm hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors"
              >
            ...
            <div class="flex gap-2">
              <button
                type="button"
                (click)="closed.emit()"
                class="rounded-lg border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
