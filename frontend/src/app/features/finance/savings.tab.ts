      ...
      <div class="grid gap-6 lg:grid-cols-3 mb-6">
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 lg:col-span-2">
          ...
          <p class="text-3xl font-semibold mt-2 tabular-nums">
            {{ eur(o.saved_total) }}
            <span class="text-slate-600 dark:text-slate-400 text-xl font-normal">/ {{ eur(o.target_total) }} {{ 'savings.saved' | translate }}</span>
          </p>
          <p class="mt-1 text-sm" [class]="o.delta_total >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'">
            {{ o.delta_total >= 0 ? '+' : '' }}{{ eur(o.delta_total) }} {{ 'savings.vsGoal' | translate }}
            ({{ o.months.length }} × {{ eur(o.monthly_target) }})
          </p>
          <div class="h-3 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden mt-4">
            <div
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          ...
          <div class="space-y-3">
            <div>
              <label for="savTarget" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'savings.monthlyTarget' | translate }}</label>
            ...
            <div>
              <label for="savStart" class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ 'savings.trackingStarts' | translate }}</label>
              ...
              <tbody>
                @for (m of monthsNewestFirst(); track m.month) {
                  <tr class="border-t border-slate-200/70 dark:border-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    <td class="px-5 py-2.5 text-slate-700 dark:text-slate-300 tabular-nums">
                      {{ m.month.slice(0, 7) }}
                      @if (m.is_current) {
                        <span class="ml-2 rounded-full bg-indigo-50 dark:bg-indigo-950 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 text-xs">{{ 'savings.inProgress' | translate }}</span>
                      }
                    </td>
                    <td class="px-5 py-2.5 text-right tabular-nums text-slate-700 dark:text-slate-300">{{ eur(m.income) }}</td>
                    <td class="px-5 py-2.5 text-right tabular-nums text-slate-700 dark:text-slate-300">{{ eur(m.expenses) }}</td>
                    <td class="px-5 py-2.5 text-right tabular-nums font-medium">{{ eur(m.saved) }}</td>
                    <td class="px-5 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-400">{{ eur(m.target) }}</td>
                    <td class="px-5 py-2.5 text-right tabular-nums font-medium"
                      [class]="m.delta >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'">
