      ...
      <div class="grid gap-4 sm:grid-cols-3 mb-6">
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          ...
          <p class="text-xs text-slate-500 mt-1">
            {{ eur(p.recurring_income_total) }} recurring + {{ eur(p.one_off_income_total) }} one-off
          </p>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <p class="text-sm text-slate-400">Fixed expenses</p>
          <p class="text-2xl font-semibold mt-1">{{ eur(p.fixed_expense_total) }}</p>
          <p class="text-xs text-slate-500 mt-1">{{ p.fixed_items.length }} recurring expenses</p>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <p class="text-sm text-slate-400">Available for variable spending</p>
          ...
          <p class="text-xs text-slate-500 mt-1">{{ eur(p.variable_expense_total) }} already spent</p>
        </div>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <div class="flex items-center justify-between mb-1">
            <h2 class="font-semibold">Fixed expenses</h2>
            <span class="text-sm text-slate-400">
              {{ p.fixed_paid_count }} of {{ p.fixed_items.length }} paid
            </span>
          </div>
          ...
          @if (p.fixed_items.length === 0) {
            <p class="text-sm text-slate-500 py-6 text-center">
              No fixed expenses this month. Add a recurring transaction to plan them.
            </p>
          }
          <ul class="space-y-2">
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <div class="flex items-center justify-between mb-4">
            <h2 class="font-semibold">Recurring transactions</h2>
          ...
          @if (recurring().length === 0) {
            <p class="text-sm text-slate-500 py-6 text-center">
              No recurring transactions yet — e.g. salary, rent, insurance.
            </p>
          }
          <ul class="space-y-2">
              ...
              <li class="rounded-lg border border-slate-800 px-3 py-2" [class.opacity-50]="!rec.is_active">
                <div class="flex items-center justify-between gap-3">
                  <div class="min-w-0">
                    <p class="truncate">
                      {{ rec.description }}
                      @if (!rec.is_active) { <span class="text-xs text-slate-500">(paused)</span> }
                    </p>
                    <p class="text-xs text-slate-500">
                      Day {{ rec.day_of_month }} · since {{ rec.start_month.slice(0, 7) }}
                      @if (rec.end_month) { · until {{ rec.end_month.slice(0, 7) }} }
                      @if (skippedThisMonth(rec)) { · <span class="text-amber-400">skipped this month</span> }
                    </p>
                  </div>
                  <div class="flex items-center gap-2 shrink-0">
                    ...
                    @if (skippedThisMonth(rec)) {
                      <button (click)="unskip(rec)" class="text-xs text-slate-400 hover:text-slate-200 border border-slate-700 rounded-md px-2 py-1" title="Restore this month's transaction">
                        Unskip
                      </button>
                    } @else {
                      <button (click)="skip(rec)" class="text-xs text-slate-400 hover:text-amber-400 border border-slate-700 rounded-md px-2 py-1" title="Skip this month">
                        Skip
                      </button>
                    }
                    <button (click)="openEdit(rec)" class="text-slate-500 hover:text-slate-200 px-1" title="Edit">✎</button>
                    <button (click)="remove(rec)" class="text-slate-500 hover:text-red-400 px-1" title="Delete template">✕</button>
                  </div>
                </div>
              </li>
            }
...
export class MonthlyPlanTab {
  ...
  load(): void {
    ...
    forkJoin({
      ...
      error: (err) => {
        if (seq !== this.loadSeq) return;
        this.loading.set(false);
        this.error.set(extractError(err, 'Could not load the monthly plan.'));
      },
    });
  }
  ...
  skip(rec: RecurringTransaction): void {
    if (!confirm(`Skip "${rec.description}" for this month? The generated transaction will be removed.`)) return;
    this.api.skipRecurringMonth(rec.id, this.monthStart()).subscribe({
      next: () => this.load(),
      error: (err) => this.error.set(extractError(err, 'Could not skip the month.')),
    });
  }
  ...
  unskip(rec: RecurringTransaction): void {
    this.api.unskipRecurringMonth(rec.id, this.monthStart()).subscribe({
      next: () => this.load(),
      error: (err) => this.error.set(extractError(err, 'Could not unskip the month.')),
    });
  }
  ...
  remove(rec: RecurringTransaction): void {
    if (!confirm(`Delete the recurring transaction "${rec.description}"? Already generated transactions are kept.`)) return;
    this.api.deleteRecurring(rec.id).subscribe({
      next: () => this.load(),
      error: (err) => this.error.set(extractError(err, 'Could not delete the template.')),
    });
  }
}
